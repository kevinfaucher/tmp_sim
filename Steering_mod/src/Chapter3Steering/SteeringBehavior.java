...
/////////////////////////////////////////////////////////////////////////////// START OF BEHAVIORS
	...
	private Vector2D Seek(Vector2D TargetPos) {
		Vector2D DesiredVelocity = mul(Vec2DNormalize(sub(TargetPos, m_pVehicle.Pos())), m_pVehicle.MaxSpeed());
	...
	private Vector2D Flee(Vector2D TargetPos) {
		...
		// squared space.
		/*
		 * const double PanicDistanceSq = 100.0f * 100.0; if
		 * (Vec2DDistanceSq(m_pVehicle.Pos(), target) > PanicDistanceSq) { return new
		 * Vector2D(0,0); }
		 */

		Vector2D DesiredVelocity = mul(Vec2DNormalize(sub(m_pVehicle.Pos(), TargetPos)), m_pVehicle.MaxSpeed());
	...
	private Vector2D Wander() {
		...
		m_vWanderTarget.add(new Vector2D(RandomClamped() * JitterThisTimeSlice, RandomClamped() * JitterThisTimeSlice));
		...
		Vector2D Target = PointToWorldSpace(target, m_pVehicle.Heading(), m_pVehicle.Side(), m_pVehicle.Pos());
	...
	private Vector2D WallAvoidance(final List<Wall2D> walls) {
		...
		Vector2D SteeringForce = new Vector2D(), point = new Vector2D(), // used for storing temporary info
		...
		for (int flr = 0; flr < m_Feelers.size(); ++flr) {
			...
			for (int w = 0; w < walls.size(); ++w) {
				if (LineIntersection2D(m_pVehicle.Pos(), m_Feelers.get(flr), walls.get(w).From(), walls.get(w).To(),
						DistToThisIPRef, point)) {
	...
	private Vector2D SeparationPlus(final List<Vehicle> neighbors) {
		...
		// iterate through the neighbors and sum up all the position vectors
		for (BaseGameEntity pV = m_pVehicle.World().CellSpace().begin(); !m_pVehicle.World().CellSpace()
				.end(); pV = m_pVehicle.World().CellSpace().next()) {
			// make sure this agent isn't included in the calculations and that
	...
	private Vector2D AlignmentPlus(final List<Vehicle> neighbors) {
		...
		// iterate through the neighbors and sum up all the position vectors
		for (MovingEntity pV = m_pVehicle.World().CellSpace().begin(); !m_pVehicle.World().CellSpace()
				.end(); pV = m_pVehicle.World().CellSpace().next()) {
			// make sure *this* agent isn't included in the calculations and that
			// the agent being examined is close enough
			if (pV != m_pVehicle) {
	 ...
	 */
	private Vector2D GetHidingPosition(final Vector2D posOb, final double radiusOb, final Vector2D posHunter) {
	 ...
	 * leader vehicle
	 */
	private Vector2D OffsetPursuit(final Vehicle leader, final Vector2D offset) {
...
//---------------------------------dtor ----------------------------------
	...
	public void RenderAids() {
		...
		if (On(behavior_type.wander) && m_pVehicle.World().RenderWanderCircle()) {
			...
			gdi.RedPen();
			gdi.Circle(PointToWorldSpace(
					mul(add(m_vWanderTarget, new Vector2D(m_dWanderDistance, 0)), m_pVehicle.BRadius()),
					m_pVehicle.Heading(), m_pVehicle.Side(), m_pVehicle.Pos()), 3);
		}
		...
		if (m_pVehicle.World().RenderDetectionBox()) {
			...
			if (!m_pVehicle.isSmoothingOn()) {
				...
				box = WorldTransform(box, m_pVehicle.Pos(), m_pVehicle.SmoothedHeading(),
	 ...
	 * acumulates their forces until the max steering force magnitude is reached, at
	 * which time the function returns the steering force accumulated to that point
	 */
	private Vector2D CalculatePrioritized() {
		Vector2D force = new Vector2D();

		if (On(behavior_type.wall_avoidance)) {
			force = mul(WallAvoidance(m_pVehicle.World().Walls()), m_dWeightWallAvoidance);
		...
		if (On(behavior_type.obstacle_avoidance)) {
			force = mul(ObstacleAvoidance(m_pVehicle.World().Obstacles()), m_dWeightObstacleAvoidance);
	...
	private Vector2D CalculateWeightedSum() {
		if (On(behavior_type.wall_avoidance)) {
			m_vSteeringForce.add(mul(WallAvoidance(m_pVehicle.World().Walls()), m_dWeightWallAvoidance));
		}

		if (On(behavior_type.obstacle_avoidance)) {
			m_vSteeringForce.add(mul(ObstacleAvoidance(m_pVehicle.World().Obstacles()), m_dWeightObstacleAvoidance));
		}
	...
	/**
	 * this method sums up the active behaviors by assigning a probabilty of being
	 * calculated to each behavior. It then tests the first priority to see if it
	 * should be calcukated this simulation-step. If so, it calculates the steering
	...
	private Vector2D CalculateDithered() {
		...
		if (!isSpacePartitioningOn()) {
			if (On(behavior_type.separation) && RandFloat() < Prm.prSeparation) {
				m_vSteeringForce
						.add(mul(Separation(m_pVehicle.World().Agents()), m_dWeightSeparation / Prm.prSeparation));
			...
			if (On(behavior_type.separation) && RandFloat() < Prm.prSeparation) {
				m_vSteeringForce
						.add(mul(SeparationPlus(m_pVehicle.World().Agents()), m_dWeightSeparation / Prm.prSeparation));
		...
		if (!isSpacePartitioningOn()) {
			if (On(behavior_type.allignment) && RandFloat() < Prm.prAlignment) {
				m_vSteeringForce.add(mul(Alignment(m_pVehicle.World().Agents()), m_dWeightAlignment / Prm.prAlignment));
			...
			if (On(behavior_type.cohesion) && RandFloat() < Prm.prCohesion) {
				m_vSteeringForce.add(mul(Cohesion(m_pVehicle.World().Agents()), m_dWeightCohesion / Prm.prCohesion));
			...
			if (On(behavior_type.allignment) && RandFloat() < Prm.prAlignment) {
				m_vSteeringForce
						.add(mul(AlignmentPlus(m_pVehicle.World().Agents()), m_dWeightAlignment / Prm.prAlignment));
			...
			if (On(behavior_type.cohesion) && RandFloat() < Prm.prCohesion) {
				m_vSteeringForce
						.add(mul(CohesionPlus(m_pVehicle.World().Agents()), m_dWeightCohesion / Prm.prCohesion));
		...
		if (On(behavior_type.arrive) && RandFloat() < Prm.prArrive) {
			m_vSteeringForce
					.add(mul(Arrive(m_pVehicle.World().Crosshair(), m_Deceleration), m_dWeightArrive / Prm.prArrive));
